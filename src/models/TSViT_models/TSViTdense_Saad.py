...
class TSViT(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        num_patches = self.num_patches_1d ** 2
        #patch_dim = (model_config['num_channels'] - 1) * self.patch_size ** 2  # -1 is set to exclude time feature        
        patch_dim = (model_config['num_channels']) * self.patch_size ** 2  # not doing -1 because time feature is passed separately
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b t c (h p1) (w p2) -> (b h w) t (p1 p2 c)', p1=self.patch_size, p2=self.patch_size),
            nn.Linear(patch_dim, self.dim),)
    ...
    def forward(self, x, xt):
        #x = x.permute(0, 1, 4, 2, 3)
        B, T, C, H, W = x.shape
        #print("xt before mutiply", xt)
        #print("xt shape before mutiply", xt.shape)
        #xt = x[:, :, -1, 0, 0]
        #x = x[:, :, :-1]
        #xt = (xt * 365.0001).to(torch.int64)
        xt = xt.to(torch.int64)
        #print("xt after mutiply", xt)
        #print("xt shape after mutiply", xt.shape)
        assert torch.all(xt < 366) and torch.all(xt >= 0), "xt contains invalid indices for one-hot encoding"
